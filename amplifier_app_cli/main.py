...
def _streaming_overlay_active(session) -> bool:
    ...
    Any exception → False (defensive; never raises).
    """
    try:
        ...
        for entry in cfg.get("hooks", []) or []:
            mod = (
                entry.get("module", "")
                if isinstance(entry, dict)
                else getattr(entry, "module", "")
            )
            if isinstance(mod, str) and (
                "streaming-ui" in mod or "streaming_ui" in mod
            ):
                ecfg = (
                    entry.get("config", {})
                    if isinstance(entry, dict)
                    else getattr(entry, "config", {})
                ) or {}
    ...
    async def _execute_with_interrupt(prompt_text: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                if not _streaming_overlay_active(session):
                    render_message(
