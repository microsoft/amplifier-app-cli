...
# talk to the human). Records durable attention advisories; it never enters the
# orchestrator session or performs manager work.
#
...
SESSION_ID=${3:?SESSION_ID required for compatible invocation; recorded only}
INTERVAL=${4:-300}
MAX_HOURS=${5:-12}
...
# Heartbeats are advisory timing observations only. They cannot prove whether a
# manager is idle or busy during a long delegate, so they never suppress a
# durable advisory.
...
# BATCH_DIR/escalation-needed and record an escalation advisory.
...
START_TS=$(date +%s)
# Grace after watchdog start identifies an ABSENT heartbeat as startup timing in
# the log. It does not defer an advisory or prove the manager is idle or busy.
...
wake() {
  local reason="$1" now within_log_gap=0
  now=$(date +%s)
  [ $(( now - last_wake )) -lt "$WAKE_GAP" ] && within_log_gap=1
  # Every observed trigger is durable, including repeats within the verbosity
  # gap. The manager's bounded snapshot and last-processed line distinguish
  # observations; this watchdog never consumes them.
  printf '%s\t%s\n' "$(date -u +%FT%TZ)" "$reason" >> "$BATCH_DIR/wake-needed"
  last_wake=$now
  [ "$within_log_gap" = 1 ] && return 0
  if [ "${escalate:-0}" = 1 ]; then
    log "ESCALATIONDEFERRED (ineffective=$ineffective live=$live width=$WIDTH): $reason"
  else
    log "WAKEDEFERRED: $reason"
  fi
  # Recording is the complete watchdog action; the gap limits log verbosity,
  # never the append-only record.
}
...
while :; do
  if [ "$MAX_POLLS" -gt 0 ] && [ "${polls:-0}" -ge "$MAX_POLLS" ]; then
  ...
  # A fresh heartbeat or startup grace provides timing context, not evidence
  # that the manager is idle or busy. Record the observation and let the
  # manager decide when it next reads wake-needed.
  if { [ "$hb_age" -ge 0 ] && [ "$hb_age" -lt "$ACTIVE_WINDOW" ]; } \
     || { [ "$hb_age" -lt 0 ] && [ "$wd_uptime" -lt "$GRACE" ]; }; then
    log "heartbeat advisory (hb_age=${hb_age}s, uptime=${wd_uptime}s)"
  fi
  # Heartbeat absent beyond the grace window = manager never checked in =
  # treat as stale so the advisory below is eligible.
  ...
  # Advance state after evaluation so a timing advisory cannot consume an ended
  # lane transition before normal trigger and rate-limit handling.
  printf '%s\n' "$ended_now" > "$STATE"
done
