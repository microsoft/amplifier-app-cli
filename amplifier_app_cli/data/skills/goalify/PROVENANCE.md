...
| L7 — work-tracker terminal verb that can refuse | Two lanes in one batch, each ending only in `work_resolve`/`work_release` on a one-item/many-lanes container item a sibling session had already resolved and held. `j1e6-ci-tool-web`: **855 turns / $202.91** against a $0 spend authority, after its deliverable (amplifier-module-tool-web PR #17) had merged hours earlier. `hd-browser-bridge`: **~887 turns / ~$184**, likewise post-merge. Both repeated one line every turn — *"No verified successful `work_resolve` or `work_release` appears in the available transcript, so the required terminal outcome is not established."* — because neither verb can succeed for a session that never held the item. The item's own errata trail shows at least 3 earlier lanes hitting the same wall and each independently rediscovering `work_erratum`; L7 exists to stop it being rediscovered a fifth time. |
