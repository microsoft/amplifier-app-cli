...
class SessionConfig:
    ...
    def is_bundle_mode(self) -> bool:
        ...
        return (
            self.profile_name.startswith("bundle:") and self.prepared_bundle is not None
        )
...
def register_mention_handling(
    session: AmplifierSession, *, bundle_mode: bool = False
) -> None:
    ...
    session.coordinator.register_capability(
        "mention_deduplicator", mention_deduplicator
    )
...
async def _process_mentions(session: AmplifierSession, profile_name: str) -> None:
    ...
    try:
        ...
        if hasattr(profile, "markdown_body") and profile.markdown_body:
            ...
            if mentions:
                mention_resolver = session.coordinator.get_capability(
                    "mention_resolver"
                )
                deduplicator = session.coordinator.get_capability(
                ...
                if mention_resolver:
                    for mention in mentions:
                        try:
                            ...
                            logger.warning(
                                "Failed to resolve mention %s: %s", mention, e
                            )
    except Exception as e:
        logger.debug(
            "Could not load profile %s for @mention processing: %s", profile_name, e
        )
